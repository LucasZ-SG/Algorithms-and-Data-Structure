...
public class ExternalChainingHashMap<K, V> {
    ...
    public V remove(K key) {
        ...
        while(node.getNext() != null){
            if(node.getNext().getKey().equals(key)){
                V oldValue = node.getNext().getValue();
    ...
    private void resizeBackingTable(int length) {
        ...
        for(ExternalChainingMapEntry<K,V> node: table){
            while(node != null){
                int newIndex = Math.abs(node.getKey().hashCode() % length);
                ExternalChainingMapEntry<K,V> newNode = newTable[newIndex];
                newTable[newIndex] = new ExternalChainingMapEntry<K,V>(node.getKey(), node.getValue());
                newTable[newIndex].setNext(newNode);
                node = node.getNext();
            }
        }
        table = newTable;
